...
type Props = {
  ...
  onClose: () => void
  url: string
  /** Optional display name above the QR */
  title?: string
  subtitle?: string
  /** Enable center logo upload */
  allowLogo?: boolean
  zIndexClass?: string
  onCopyLink?: () => void
...
export default function VCardQrModal({
  open,
  onClose,
  url,
  title = 'vCard QR Code',
  ...
  // Reset transient UI flags when the modal opens (render-time state adjust).
  if (open !== wasOpen) {
    setWasOpen(open)
    if (open) {
      setCopied(false)
