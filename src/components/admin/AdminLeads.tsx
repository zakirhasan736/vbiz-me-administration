...
export default function AdminLeads() {
  ...
  const saves = useMemo(() => savesRaw.map(mapAdminLeadRow), [savesRaw])
  const notes = useMemo(() => notesRaw.map(mapAdminLeadRow), [notesRaw])

  const total = stats?.totalSaves ?? saves.length
          ...
          <div className="flex w-full min-w-0 gap-1 rounded-2xl bg-slate-100 p-1 dark:bg-white/5">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                <Save className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">Saves</span>
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                <MessageCircle className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">Notes</span>
              </span>
            </button>
          </div>
        </div>
