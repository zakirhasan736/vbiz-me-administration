...
import {
  GUEST_MESSAGES_EVENT,
  GuestMessageRecord,
  getGuestMessagesForOwner,
...
/** Normalized lead shape for contact saves, guest messages, and API-mapped rows. */
...
type LeadNotesRepliesPanelProps = {
  ownerId?: string | null
  role?: RoleScope
  vCardIdFilter?: string | null
  className?: string
  /** Optional external list (corporate leads merge). Falls back to contact saves. */
  leads?: LeadNoteItem[]
  /** Optional external guest-only notes (API). Falls back to localStorage guest messages. */
  guestOnly?: LeadNoteItem[]
  loading?: boolean
...
function getGuestNote(lead: LeadNoteItem): string {
  return (lead.guestMessage || lead.message || lead.text || lead.visitorNote || '').trim()
}

function isGuestOnly(lead: LeadNoteItem): boolean {
  return lead.kind === 'guest_message' || String(lead.id || '').startsWith('gmsg_')
}

/**
 * Urgent follow-up workspace — shows guest notes, private manager notes, and replies.
 * Includes contact-saves AND message-only guest notes (no contact save).
 */
export default function LeadNotesRepliesPanel({
  ownerId,
  role = 'single',
  vCardIdFilter = null,
  className,
  leads,
  guestOnly: guestOnlyProp,
  loading = false,
  onSaveNote,
  onSendReply,
  ...
  useEffect(() => {
    if (apiMode) return
    const onUpdate = () => setStorageVersion((v) => v + 1)
    window.addEventListener(CONTACT_SAVES_EVENT, onUpdate)
    window.addEventListener(GUEST_MESSAGES_EVENT, onUpdate)
    window.addEventListener('storage', onUpdate)
    return () => {
      window.removeEventListener(CONTACT_SAVES_EVENT, onUpdate)
      window.removeEventListener(GUEST_MESSAGES_EVENT, onUpdate)
      window.removeEventListener('storage', onUpdate)
    }
  }, [apiMode])

  const records = useMemo<ContactSaveRecord[]>(() => {
    if (apiMode) return []
    // storageVersion busts the cache when localStorage events fire
    return storageVersion >= 0 ? getContactSavesForOwner(ownerId, role, vCardIdFilter) : []
  }, [apiMode, ownerId, role, vCardIdFilter, storageVersion])

  const guestOnlyLocal = useMemo<GuestMessageRecord[]>(() => {
    if (apiMode) return []
    return storageVersion >= 0 ? getGuestMessagesForOwner(ownerId, role, vCardIdFilter) : []
  }, [apiMode, ownerId, role, vCardIdFilter, storageVersion])

  const guestOnly: LeadNoteItem[] = guestOnlyProp !== undefined ? guestOnlyProp : guestOnlyLocal

  const sourceList = useMemo(() => {
    const base: LeadNoteItem[] = leads !== undefined ? leads : records
    const byId = new Map<string, LeadNoteItem>()
    ;[...base, ...guestOnly].forEach((item) => byId.set(item.id, item))
  ...
  const handleSaveNote = async (lead: LeadNoteItem) => {
    const id = lead.id
    const text = noteMap[id] ?? lead.privateNotes ?? lead.notes ?? ''
    if (onSaveNote) {
      await onSaveNote(id, text, lead)
      return
    }
    if (isGuestOnly(lead)) {
      if (updateGuestMessageNotes(id, text)) {
        bumpLocal()
  ...
  const handleSendReply = async (lead: LeadNoteItem) => {
    const id = lead.id
    const text = (replyMap[id] || '').trim()
    if (!text) return
    if (onSendReply) {
      await onSendReply(lead, text)
      setReplyMap((p) => ({ ...p, [id]: '' }))
      setActiveId(null)
      return
    }
    if (isGuestOnly(lead)) {
      if (updateGuestMessageReply(id, text)) {
        setReplyMap((p) => ({ ...p, [id]: '' }))
        setActiveId(null)
        bumpLocal()
        notify.info(`Urgent reply sent to ${lead.fullName || 'guest'}.`)
      }
      return
    }
    if (updateContactSaveReply(id, text)) {
      setReplyMap((p) => ({ ...p, [id]: '' }))
      setActiveId(null)
      bumpLocal()
      notify.info(`Urgent reply sent to ${lead.fullName || lead.name || 'lead'}.`)
    }
  }
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="min-w-0 divide-y divide-rose-100/50 overflow-x-hidden overflow-y-visible dark:divide-white/5">
          {filtered.map((lead) => {
            const id = lead.id
            const open = activeId === id
            const name = lead.fullName || lead.name || 'Unnamed'
            const noteValue = noteMap[id] !== undefined ? noteMap[id] : lead.privateNotes || lead.notes || ''
            const guestNote = getGuestNote(lead)
            const needsReply = !lead.lastReply
            const messageOnly = isGuestOnly(lead)
            return (
              <div
                key={id}
                className={cn(
                  ...
                  <div className="min-w-0 overflow-hidden">
                    ...
                    <div className="mt-1 min-w-0 space-y-0.5 text-[11px] font-semibold text-slate-400">
                      ...
                      <p className="truncate">
                        {lead.phoneNumber || lead.phone || 'No phone'} · {lead.vCardName || 'vCard'}
                      </p>
                    </div>
                    {guestNote ? (
                      <p
                    ...
                    <div className="grid min-w-0 grid-cols-1 gap-3 sm:gap-4">
                      <div className="min-w-0 rounded-2xl border border-slate-200/80 bg-white p-3.5 sm:p-4 dark:border-white/10 dark:bg-[#070a13]">
                        ...
                        <button
                          ...
                          className="mt-2 w-full rounded-lg bg-slate-900 px-3 py-2 text-[10px] font-black tracking-wider text-white uppercase sm:w-auto dark:bg-white/10"
                        >
                          Save Note
                        </button>
                      </div>
                      ...
                      <div className="min-w-0 rounded-2xl border border-rose-200/60 bg-rose-50/50 p-3.5 sm:p-4 dark:border-rose-500/20 dark:bg-rose-500/5">
                        ...
                        <button
                          ...
                          className="mt-2 w-full rounded-xl bg-rose-600 px-4 py-2.5 text-[10px] font-black tracking-wider text-white uppercase hover:bg-rose-700 sm:w-auto"
                        >
                          Send Urgent Reply
                        </button>
                      </div>
