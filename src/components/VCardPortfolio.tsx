...
export function TabPortfolio() {
  ...
  const addPortfolio = () => {
    ...
    setPortfolios([...portfoliosRef.current, next])
    expandNew(next.id)
  }
          ...
          <div className="space-y-4">
            ...
            <div className="mt-8 flex flex-col items-center gap-4 pt-6">
              <button
                type="button"
                onClick={addPortfolio}
                className={cn(bottomAddButtonClass, 'text-teal-600 hover:border-teal-500/30 dark:text-teal-400')}
              >
                <Plus className="h-4 w-4" /> Add Another Portfolio
              </button>
            </div>
          </div>
        )}
