...
import { Calendar, FileBox, LayoutGrid, Plus, Trash2 } from 'lucide-react'
...
export function BlogEditorPanel({ posts: rawPosts, onPostsChange, profileId }: BlogEditorPanelProps) {
  ...
  const addPost = () => {
    setPosts([createDefaultGeneralPost(), ...postsRef.current])
  }

  const removePost = (id: string) => {
    setPosts(postsRef.current.filter((p) => p.id !== id))
  }
  ...
  return (
    <div className="animate-in fade-in mx-auto flex h-full w-full max-w-3xl flex-col space-y-6 pb-12 duration-500">
      ...
      {posts.length === 0 ? (
        ...
        <ReorderList
          ...
          renderItem={(item, idx, controls) => (
            <div
              id={`entry-${item.id}`}
              className="scroll-mt-24 space-y-4 rounded-2xl border border-slate-200 bg-slate-50/50 p-5 dark:border-white/10 dark:bg-white/2"
            >
              <div className="flex items-center justify-between gap-3">
                <p className="text-[11px] font-black tracking-wider text-slate-400 uppercase">Post {idx + 1}</p>
      ...
      <button
        type="button"
        onClick={addPost}
        className="inline-flex items-center gap-2 rounded-xl border border-dashed border-indigo-300 px-4 py-2.5 text-sm font-bold text-indigo-700 dark:border-indigo-500/40 dark:text-indigo-300"
      >
        <Plus className="h-4 w-4" /> Add post
      </button>
    </div>
