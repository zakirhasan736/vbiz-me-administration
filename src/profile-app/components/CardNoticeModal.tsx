...
export function CardNoticeModal({ open, notice, ownerName, onClose }: Props) {
  ...
  return (
    <ProfileModalShell
      isOpen={open}
      onClose={onClose}
      backdropClassName="vbiz-modal-backdrop fixed inset-0 z-250 flex items-center justify-center px-[max(0.75rem,env(safe-area-inset-left,0px))] pt-[max(5dvh,env(safe-area-inset-top,0px))] pr-[max(0.75rem,env(safe-area-inset-right,0px))] pb-[max(5dvh,env(safe-area-inset-bottom,0px))] backdrop-blur-md"
      panelClassName="flex max-h-[90dvh] w-full max-w-md flex-col overflow-x-hidden overflow-y-auto overscroll-contain rounded-2xl border border-transparent bg-transparent shadow-none"
    >
