...
export function CertificateImageLightbox({
  ...
  return createPortal(
    <motion.div
      ...
      className="vbiz-modal-backdrop fixed inset-0 z-200 flex items-center justify-center bg-zinc-950/75 pt-[max(5dvh,env(safe-area-inset-top,0px))] pr-[max(0.75rem,env(safe-area-inset-right,0px))] pb-[max(5dvh,env(safe-area-inset-bottom,0px))] pl-[max(0.75rem,env(safe-area-inset-left,0px))] backdrop-blur-sm sm:px-6"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
      ...
      >
        <div className="overflow-hidden rounded-lg bg-white shadow-2xl">
          {kind === 'image' && mediaUrl && isUsableImageSrc(mediaUrl) ? (
            ...
            <img src={mediaUrl} alt={preview.title} className="max-h-[90dvh] w-full object-contain" />
          ) : kind === 'pdf' && mediaUrl ? (
