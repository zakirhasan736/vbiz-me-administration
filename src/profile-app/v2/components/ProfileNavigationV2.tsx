...
export function ProfileNavigationV2({ slugForPersistence, embedded }: ProfileNavigationV2Props) {
  ...
  return (
    <>
      {!embedded ? <PublicAnnouncementOverlay slot="v2-under-nav" /> : null}
      <div
        className={cn(
          'vbiz-floating-nav sticky top-4 z-50 mb-8 flex w-full max-w-full flex-col items-center px-3 sm:top-6 sm:px-0',
          embedded ? 'max-w-full' : ''
        )}
      >
        <motion.div
        ...
        >
          <div className="min-w-0 flex-1 overflow-visible">
            <div
              ...
              className={cn(
                'vbiz-floating-nav-scroll mask-edges items-center gap-1.5 px-2 sm:gap-2',
                navScrollClassName
              )}
            >
