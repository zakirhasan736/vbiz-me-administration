/**
 * Canonical tab registry (mirrors backend `tabRegistry.ts`).
 * Internal keys are snake_case. HTTP routes and public section names are separate.
 */
...
export const TAB_REGISTRY: Record<string, TabRegistryEntry> = {
  services: list('services', 'Services', 'services', 'services', 1, 'service', 'services'),
  clients: list('clients', 'Clients', 'clients', 'clients', 2, 'tab_item', 'clients'),
  reviews: list('reviews', 'Reviews', 'reviews', 'reviews', 3, 'review', 'reviews'),
  gallery: list('gallery', 'Gallery', 'portfolios', 'gallery', 4, 'portfolio', 'gallery'),
  videos: list('videos', 'Video', 'videos', 'video', 5),
  blogs: list('blogs', 'Blog', 'blogs', 'blog', 6, 'blog', 'blog'),
  ...
  faqs: list('faqs', 'FAQ', 'faqs', 'Faq', 13, 'tab_item', 'faq'),
...
export function isDirectTab(key: string): boolean {
  return TAB_REGISTRY[key]?.architecture === 'direct'
}
